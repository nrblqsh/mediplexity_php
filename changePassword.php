...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $input !== null) {
    if (isset($input['patientID']) && isset($input['oldPassword']) && isset($input['newPassword'])) {
        ...
        try {
            ...
            $stmtFetch = $conn->prepare("SELECT password FROM patient WHERE patientID = ?");
            $stmtFetch->bind_param("i", $patientID);
            $stmtFetch->execute();
            ...
            if ($oldPassword === $currentPassword) {
                ...
                $stmtUpdate = $conn->prepare("UPDATE patient SET password = ? WHERE patientID = ?");
                $stmtUpdate->bind_param("si", $newPassword, $patientID);
